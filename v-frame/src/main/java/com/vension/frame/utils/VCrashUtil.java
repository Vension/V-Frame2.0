...
public class VCrashUtil {
	...
	static {
		try {
			PackageInfo pi = VFrame.getApplication().getPackageManager().getPackageInfo(VFrame.getApplication().getPackageName(), 0);
			if (pi != null) {
		...
		UNCAUGHT_EXCEPTION_HANDLER = new Thread.UncaughtExceptionHandler() {
			...
			public void uncaughtException(final Thread t, final Throwable e) {
				...
				String fileName = FORMAT.format(now) + "Bug.log";
	...
	public static void init(final String crashDir) {
		...
		if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
				&& VFrame.getApplication().getExternalCacheDir() != null)
			defaultDir = VFrame.getApplication().getExternalCacheDir() + FILE_SEP + "crash" + FILE_SEP;
		else {
			defaultDir = VFrame.getApplication().getCacheDir() + FILE_SEP + "crash" + FILE_SEP;
		}
