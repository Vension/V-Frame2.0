...
import com.vension.frame.http.other.HttpCallBack;
import com.vension.frame.http.other.IHttpEngine;
import com.vension.frame.http.other.VHttp;
...
public class OKHttpEngine implements IHttpEngine {
    ...
    public OKHttpEngine() {
        client = new OkHttpClient().newBuilder()
                ...
                .cache(new Cache(VFrame.getContext().getCacheDir(), cacheSize))
                .build();
    }
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, final Response response) throws IOException {
                if (response.isSuccessful()) {
                    ...
                    VHttp.handler.post(new Runnable() {
                        @Override
                        public void run() {
                            //我这里使用的是fastjson，你也可以用gson，jackjson等
//                            callBack.onSuccess(JSON.parseObject(result, V_Http.analysisClassInfo(callBack)));
                            callBack.onSuccess(new Gson().toJson(result, VHttp.analysisClassInfo(callBack)));
                        }
                    });
                } else {
                    VHttp.handler.post(new Runnable() {
