 ...
 * 短信的Uri共有一下几种：
   ...
   content://sms/queued    待发送列表  (比如开启飞行模式后，该短信就在待发送列表里)需要权限
...
public class SmsObserver extends ContentObserver {
	...
	public SmsObserver(Activity activity, Handler handler, String like, SmsListener listener) {
		super(handler);
	...
	public void onChange(boolean selfChange) {
		...
		if (cursor != null) {
			...
			if (cursor.moveToFirst()) {
				...
				if (!VEmptyUtil.isSpace(smsContent)) {
